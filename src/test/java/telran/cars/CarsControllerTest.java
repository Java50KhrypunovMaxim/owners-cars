...
class CarsControllerTest {
	...
	private static final String PERSON_ALREADY_EXISTS = "person already exists";
	...
	void testPersonAlreadyExists() throws Exception {
	    ...
	    String actualJSON = mockMvc.perform(post("http://localhost:8080/cars/person")
	...
	@Test
	void testСarAlreadyExists() throws Exception {
		when(carsService.addCar(carDto)).thenThrow(new IllegalStateException(PERSON_ALREADY_EXISTS));
	    String jsonCarDto = mapper.writeValueAsString(carDto); 
	    String actualJSON = mockMvc.perform(post("http://localhost:8080/cars")
	            .contentType(MediaType.APPLICATION_JSON)
	            .content(jsonCarDto))
	            .andExpect(status().isBadRequest())
	            .andReturn().getResponse().getContentAsString();
